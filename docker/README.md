...
The docker image will run foooothm-bot with a SQLite database and
...
* Choose whether you want to use SQLite or Postgres as your database backend.
  ...
  ```
  database: "postgres://username:password@postgres/foooothm-bot?sslmode=disable"
  ```
...
```
docker-compose up foooothm-bot
```
...
```
docker-compose pull foooothm-bot
```
...
[foooothm-bot.service](foooothm-bot.service). The service uses
`docker-compose` to start and stop the bot.

Copy the file to `/etc/systemd/system/foooothm-bot.service` and edit to
match your setup. You can then start the bot with:

```
systemctl start foooothm-bot
```

and stop it with:

```
systemctl stop foooothm-bot
```

To run the bot on system startup:

```
systemctl enable foooothm-bot
```
...
```
docker build -t somebody/foooothm-bot:latest -f docker/Dockerfile .
```
